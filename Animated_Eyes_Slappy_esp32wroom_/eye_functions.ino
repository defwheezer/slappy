...
void frame(uint16_t iScale) // Iris scale (0-1023)
{
  ...
  bool talk = digitalRead(TALKING_PIN);
  bool awake = digitalRead(AWAKE_PIN);
...
//if talking pin will be high
talk = digitalRead(TALKING_PIN);
#if (talk)
  // look striaght ahead
  int16_t dx, dy;
  int32_t d;
  eyeX = 511; // Raw (unclipped) X/Y reading
  eyeY = 511;
...
//*****************************************************
//if talking, over-ride all previous and center eyes
talk = digitalRead(TALKING_PIN);
awake = digitalRead(AWAKE_PIN);
if(talk) {
...
if(!awake){ 
  ...
  analogWrite(DISPLAY_BACKLIGHT, LOW);
}
else {
